...
//
void TopologyWorker::setPartList(std::vector<TLorentzVector>& e1,
                                 std::vector<TLorentzVector>& e2)
{
    ...
    for (Int_t elem = 0; elem < numElements2; elem++)
    {
        ...
        // cout << "mom2: " << mom2(np2, 1) << ", " << mom2(np2, 2) << ", "
        //      << mom2(np2, 3) << ", " << mom2(np2, 4) << endl;
    ...
    for (Int_t pass = 1; pass < 3; pass++)
    {
        ...
        for (Int_t iw = 0; iw < TMath::Min(nc, 10) && nagree < m_iGood; iw++)
        {
            ...
            while (thp > thps + m_dConv)
            {
                ...
                thp = TMath::Sqrt(tpr[1] * tpr[1] + tpr[2] * tpr[2]
                                  + tpr[3] * tpr[3])
                      / tmax;
            }
    ...
    for (Int_t i = 0; i < np; i++)
    {
        thp +=
            mom(i, 5)
            * TMath::Abs(m_dAxes(3, 1) * mom(i, 1) + m_dAxes(3, 2) * mom(i, 2));
    }
...
void TopologyWorker::ludbrb(
    TMatrix* mom, double the, double phi, double bx, double by, double bz)
{
...
void TopologyWorker::sanda()
{
    ...
    SQ = (SM[1][1] * SM[2][2] + SM[1][1] * SM[3][3] + SM[2][2] * SM[3][3]
          - pow(SM[1][2], 2) - pow(SM[1][3], 2) - pow(SM[2][3], 2))
             / 3.
         - 1. / 9.;
    SR = -0.5
             * (SQ + 1. / 9. + SM[1][1] * pow(SM[2][3], 2)
                + SM[2][2] * pow(SM[1][3], 2) + SM[3][3] * pow(SM[1][2], 2)
                - SM[1][1] * SM[2][2] * SM[3][3])
         + SM[1][2] * SM[1][3] * SM[2][3] + 1. / 27.;

    SP = TMath::Cos(TMath::ACos(TMath::Max(
                        TMath::Min(SR / TMath::Sqrt(-pow(SQ, 3)), 1.), -1.))
                    / 3.);
    ...
    if (P[N + 2][4] > 1E-5)
    {
        ...
        { // 240
            ...
            P[N + I][JB2] = SV[JC][JB1];
            P[N + I][JB] =
                -(SV[JA][JB1] * P[N + I][JB1] + SV[JA][JB2] * P[N + I][JB2])
                / SV[JA][JB];
        ...
        P[N + 2][1] =
            SGN * (P[N + 1][2] * P[N + 3][3] - P[N + 1][3] * P[N + 3][2]);
        P[N + 2][2] =
            SGN * (P[N + 1][3] * P[N + 3][1] - P[N + 1][1] * P[N + 3][3]);
        P[N + 2][3] =
            SGN * (P[N + 1][1] * P[N + 3][2] - P[N + 1][2] * P[N + 3][1]);
...
{
    //        float SPH=-1; Variables aren't used. Bad practice. Commented out
    //        so if needed, can be reintroduced. float APL=-1; Variables aren't
    ...
    // Variables SPH and APL aren't used. Bad practice. Commented out so if
    // needed, can be reintroduced.
    // C...Very low multiplicities (0 or 1) not considered.
    // if (NP < 2)
    // {
    //     SPH = -1.;
...
void TopologyWorker::fowo()
{
    ...
    if (1 == 1)
    {
        ...
        for (I = 1; I < N + 1; I++)
        {
            ...
            // p/E
            P[N + NP][4] =
                sqrt(pow(P[I][1], 2) + pow(P[I][2], 2) + pow(P[I][3], 2));
            // Et/Ettot
        ...
        H50 = 0.;
        H60 = 0.;
        for (I1 = N + 1; I1 < N + NP + 1; I1++)
        { // 130
            for (I2 = I1 + 1; I2 < N + NP + 1; I2++)
            { // 120
                CTHE = (P[I1][1] * P[I2][1] + P[I1][2] * P[I2][2]
                        + P[I1][3] * P[I2][3])
                       / (P[I1][4] * P[I2][4]);
                ...
                //       + P[I1][5] * P[I2][5]
                //             * (231 / 16. * pow(CTHE, 6)
                //                - 315. / 16. * CTHE * CTHE * CTHE * CTHE
                //                + 105. / 16. * CTHE * CTHE - 5. / 16.);
